...
def draw_network(network):
    ...
    with graph.subgraph(name="input") as c:
        ...
        for i in range(network.number_inputs):
            c.node(str(network.nodes[i].get_id()), network.nodes[i].get_tag())

    for i in network.nodes[network.number_static_nodes:]:
        graph.node(name=f"{i.get_id()}", label=f"{i.get_bias()}")
    ...
    with graph.subgraph(name="output") as c:
        ...
        for i in range(network.number_inputs, network.number_static_nodes):
            c.node(str(network.nodes[i].get_id()), network.nodes[i].get_tag(), rank="same")
    ...
    for i in network.edges:
        if i.active:
            weight = i.get_weight()
            graph.edge(str(i.get_start().get_id()), str(i.get_end().get_id()), label=str(weight))
